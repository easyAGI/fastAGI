...
def main():
    ...
    with ui.tab_panels(tabs, value=chat_tab).props('style="background-color: rgba(255, 255, 255, 0.5);"').classes('response-style'):
        ...
        # Add autonomous reasoning toggle
        def toggle_autonomous_reasoning():
            openmind.autonomous_reasoning = not openmind.autonomous_reasoning
            ui.notify(f'Autonomous reasoning: {"Enabled" if openmind.autonomous_reasoning else "Disabled"}')

        ui.switch('Autonomous Reasoning', on_change=toggle_autonomous_reasoning).classes('autonomous-switch')
