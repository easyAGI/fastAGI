...
@app.on_startup
async def startup():
    logging.debug("Starting up easyAGI")
    asyncio.create_task(openmind.main_loop())  # Start the main loop for internal reasoning
...
def main():
    ...
    async def send() -> None:
        ...
        # Start the internal reasoning loop after the first user input
        if not hasattr(openmind, 'reasoning_task') or openmind.reasoning_task is None:
    ...
    ui.switch('Autonomous Reasoning', value=False, on_change=toggle_autonomous_reasoning).classes('ml-4')
    ...
    # Initialize internal reasoning loop after the first user input
    openmind.reasoning_task = None  # Placeholder for the task
